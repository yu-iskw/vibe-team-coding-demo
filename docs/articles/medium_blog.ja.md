...
### プラグイン可能なエージェント・エコシステム：妥協のない選択

Vibe Kanbanの最も強力な機能の一つは、その **「エージェントに依存しない（Agent-agnostic）」性質** です。私たちは深い統合のために `vibe-worker` を主に使用していますが、システム自体はプラグイン可能なエコシステムとして設計されています。

**「エージェント・プロファイル (Agent Profiles)」** を通じて、開発者はワーカーの核となる「脳」を自由に入れ替えることができます。**Claude Code** の純粋な推論能力、**Gemini** のスピード、**Cursor** の親しみやすさ、あるいは **OpenAI Codex** の多才さなど、Vibe Kanbanはそれらすべてを同じ統合ボード上でオーケストレートします。

これは、あなたのワークフローが将来にわたって有効であることを意味します。より高性能な新しいモデルやコーディング・エージェントが登場した際、エンジニアリング・プロセスを変えることなく、既存の看板駆動のオーケストレーションにそれらを組み込むことができるのです。

サポートされているエージェントや設定方法の詳細については、[Vibe Kanban ドキュメント](https://www.vibekanban.com/docs/supported-coding-agents) を参照してください。
